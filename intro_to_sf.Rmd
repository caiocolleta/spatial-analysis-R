...
system.time(ak_shp_rgdal <- readOGR(dsn="shapefiles", layer="ak_regions")) #system.time shows how long it takes to load the file
object.size(ak_shp_rgdal) # object.size show the size of the file
plot(ak_shp_rgdal)
```

```{r read_shp_sf}
## Read in shapefile using sf (you can st_read as well) -> sf is faster than rgdal

system.time(ak_shp_sf <- read_sf(dsn = "shapefiles", layer="ak_regions")) # or read_sf(dsn = "shapefiles", layer = "ak_regions")
...
```{r}
st_crs(ak_shp_sf) # st_crs means coordinate reference system. EPSG: 4326 is the coordinate reference system in this file and datum = WGS84
```
...
```{r} 
# transforming the coordinate reference system to the one you are going to be using
...
```{r select}
# type this in the console (?sf::tidyverse) to help you understand how it works

ak_shp_sf %>%
  select(region, geometry) # you don't have to include geometry because the shapefile already includes it 
```
...
class(pop) # we have to change from data.frame to sf object
...
#changing from data frame to sf object

pop_sf <- st_as_sf(pop, 
                  coords = c('lng', 'lat'), # x and y (longitude and latitude)
                  crs = 4326, # coordinate reference system
                  remove = F) # transform the geometry columns to
#plot(pop_sf["population"])
head(pop_sf)
```
...
pop_region <- pop_joined_sf %>% 
  ...
  st_set_geometry(NULL) # use this to remove the geometry column
...
ggplot() +
  ...
  scale_size(range = c(0.01, 0.2), guide = F) + # guide = F turns off the legend
  theme_bw() +
  labs(fill = "Total Population") +
  scale_fill_continuous(low = "khaki", high =  "firebrick", labels = comma) #function from scales package
